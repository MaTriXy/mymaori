...
public class RotateActivity extends Activity implements ImageProcessorListener {
	...
	private EditorSaveConstants editor;
	...
	private void restoreSavedValues(final Object data) {
		...
		if (bitmapToRead == editor.RESTORE_PREVIEW_BITMAP){
	...
	public Object onRetainNonConfigurationInstance() {
		...
		if (imageProcessor.getLastResultBitmap() == null){
			saveObject.putInt("BITMAP", editor.RESTORE_SAVED_BITMAP);
		} else {
			saveObject.putInt("BITMAP",editor.RESTORE_PREVIEW_BITMAP);
		}
	...
	private OnClickListener okButtonListener = new OnClickListener() {
		public void onClick(View v) {
			setResult(RESULT_OK);
			ImageProcessor.getInstance().setBitmap(result1);
			imageProcessor.save();
			finish();
		}
	};
